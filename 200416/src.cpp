...
void OutputImage(BYTE* Img, BYTE* Out, int W, int H) // 2주차 과제 (1)
{
	int ImgSize = W * H;
...
void InverseImage(BYTE* Img, BYTE *Out, int W, int H) // 2주차 과제 (3)
{
	int ImgSize = W * H;
	for (int i = 0; i < ImgSize; i++)
	{
...
void BrightnessAdj(BYTE* Img, BYTE* Out, int W, int H, int Val) // 2주차 과제 (2) -> val 밝기값 인자
{
	int ImgSize = W * H;
	for (int i = 0; i < ImgSize; i++)
	{
...
int GozalezBinThresh(int* Histo) // 3주차 과제 Gonzalez 자동 이진화 임계치 결정
{
	BYTE Low, High;
	double mean1 = 0.0, mean2 = 0.0;
	int pix1 = 0, pix2 = 0, sum1 = 0, sum2 = 0, temp = 0, Threshold;
	for (int i = 0; i < 256; i++) {
		if (Histo[i] != 0) {
			Low = i;
			break;
	...
	do {
		...
		for (int i = Low; i <= Threshold; i++) {
			sum1 = sum1 + i * Histo[i];
			pix1 += Histo[i];
		}
		mean1 = sum1 / (double)pix1;

		for (int i = Threshold + 1; i <= High; i++) {
			sum2 = sum2 + i * Histo[i];
			pix2 += Histo[i];
		}
		mean2 = sum2 / (double)pix2;

		Threshold = (int)((mean1 + mean2) / 2.0);
		if ((Threshold - temp < 3) && (Threshold - temp > -3)) {
			break;
		}
		temp = Threshold;
	} while (1);
...
void MedianFiltering(BYTE* Img, BYTE* Out, int W, int H, int Length) { // 5주차 과제
	// Length는 마스크 한 변의 길이
		...
		for (j = Margin; j < W - Margin; j++) { // Margin을 제외한 영상의 가로 만큼 반복
			for (m = -Margin; m <= Margin; m++) { // 마스크 가장 위쪽 행부터 가장 아래쪽 행까지 반복
				for (n = -Margin; n <= Margin; n++) { // 마스크 가장 왼쪽 열부터 가장 오른쪽 열까지 반복
					temp[(m + Margin) * Length + (n + Margin)] = Img[(i + m) * W + j + n]; // 각 인덱스에 맞게 Image로부터 temp에 저장->temp에 밝기 값 저장
				}
			}
			Out[i * W + j] = Median(temp, WSize); // temp에 입력된 밝기값들 중에서 중간에 해당하는 밝기값을 마스크의 가운데에 입력
		}
	}
	free(temp); //동적 할당 해제
}
...
void DrawRectOutline(BYTE* Img, int W, int H, int LU_X, int LU_Y, int RD_X, int RD_Y) { // 6주차 과제 외접 사각형 그리는 함수
...
void DrawCrossLine(BYTE* Img, int W, int H, int Cx, int Cy) { // 6주차 과제 무게 중심을 통과하는 수평선 수직선 그리는 함수
...
int main()
{
	...
	//Scailing(Image, Output, W, H, 0.7, 0.7);
	...
	SaveBMPFile(hf, hInfo, hRGB, Output, hInfo.biWidth, hInfo.biHeight, "median5.bmp");
