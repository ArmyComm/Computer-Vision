...
int main()
{
	...
	*/
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if ((Cb[i * W + j] > 78 && Cb[i * W + j] < 127) && (Cr[i * W + j] > 130 && Cr[i * W + j] < 175)) {
				Output[i * W * 3 + j * 3] = Image[i * W * 3 + j * 3];
				Output[i * W * 3 + j * 3 + 1] = Image[i * W * 3 + j * 3 + 1];
				Output[i * W * 3 + j * 3 + 2] = Image[i * W * 3 + j * 3 + 2];
			}
	...
	for (int i = 1; i < H; i++) {
		for (int j = 1; j < W; j++) {
			if ((Output[i * W * 3 + j * 3]!=0) && (Output[i * W * 3 + j * 3 + 1] != 0) && (Output[i * W * 3 + j * 3 + 2] != 0)) { //전경화소라면
				if ((Output[(i - 1) * W*3 + j*3] == 0 && Output[(i - 1) * W * 3 + j * 3 + 1] == 0 && Output[(i - 1) * W * 3 + j * 3 + 2] == 0)
					 || (Output[(i + 1) * W * 3 + j * 3] == 0 && Output[(i + 1) * W * 3 + j * 3 + 1] == 0 && Output[(i + 1) * W * 3 + j * 3 + 2] == 0)
					 || (Output[i * W * 3 + (j - 1) * 3] == 0 && Output[i * W * 3 + (j - 1) * 3 + 1] == 0 && Output[i * W * 3 + (j - 1) * 3 + 2] == 0)
					 || (Output[i * W * 3 + (j + 1) * 3] == 0 && Output[i * W * 3 + (j + 1) * 3 + 1] == 0 && Output[i * W * 3 + (j + 1) * 3 + 2] == 0)) {
					if (i <= RD_Y) RD_Y = i;
					if (i >= LU_Y) LU_Y = i;
					if (j <= LU_X) LU_X = j;
	...
	/*for (int i = LU_X; i < RD_X; i++) {
		Output[LU_Y * W * 3 + i * 3] = 0;
		Output[LU_Y * W * 3 + i * 3 + 1] = 0;
		Output[LU_Y * W * 3 + i * 3 + 2] = 255;
	}
	for (int i = LU_X; i < RD_X; i++) {
		Output[RD_Y * W * 3 + i * 3] = 0;
		Output[RD_Y * W * 3 + i * 3 + 1] = 0;
		Output[RD_Y * W * 3 + i * 3 + 2] = 255;
	}
	for (int i = LU_Y; i < RD_Y; i++) {
		Output[i * W * 3 + LU_X * 3] = 0;
		Output[i * W * 3 + LU_X * 3 + 1] = 0;
		Output[i * W * 3 + LU_X * 3 + 2] = 255;
	}
	for (int i = LU_Y; i < RD_Y; i++) {
		Output[i * W * 3 + RD_X * 3] = 0;
		Output[i * W * 3 + RD_X * 3 + 1] = 0;
		Output[i * W * 3 + RD_X * 3 + 2] = 0;
	}*/
		...
		for (int j = 0; j < W; j++) {
			if ((j >= LU_X) && (j <= RD_X)) {
				if ((i == RD_Y) || (i == LU_Y)) {
					...
					Output[i * W * 3 + j * 3 + 2] = 255;// 사각형 윗변 아랫변 라인
				}
			}
			...
			if ((i >= RD_Y) && (i <= LU_Y)) {
				if ((j == LU_X) || (j == RD_X)) {
					Output[i * W * 3 + j * 3] = 0;
					Output[i * W * 3 + j * 3 + 1] = 0;
					Output[i * W * 3 + j * 3 + 2] = 255; // 사각형 왼쪽 오른쪽 라인
				}
			}
		}
	}
